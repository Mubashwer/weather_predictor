...
class Location < ActiveRecord::Base
  ...
  def as_json(options ={})
    my_hash = {"id" => station_id, "lat" => lat.to_s,
               "lon" => lon.to_s,
               "last_update" => nil}
    my_hash["last_update"] = last_update.strftime("%H:%M%P %d-%m-%Y") if !last_update.nil?
    return my_hash
  end
  ...
  # returns json of locations with observation data for given date
  def self.get_locs_json(locations, date)
    locs = locations.map do |loc|
      loc_json = loc.as_json
      loc_json["measurements"] = Observation.get_measurements(date, loc).as_json
      loc_json
    end
    return locs.as_json
  end
  ...
  def self.find_sorted_weather_stations lat1, lon1, range
    ...
    Location.all.each do |loc|
      ...
      if distance <= range
        distances << {loc: loc, id: loc.id, distance: distance}
      end
    end
    distances = distances.sort{ |e1, e2| e1[:distance] <=> e2[:distance] }
